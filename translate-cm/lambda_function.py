        ...
        for s3_key in fileList:
            ...
            if not s3_object.endswith("/"):
                ...
                english = translate.translate_text(
                    Text=textToTranslate, SourceLanguageCode="auto", TargetLanguageCode="en")
                sourceLang = str(english['SourceLanguageCode'])
                entities = comprehendmedical.detect_entities(
                ...
                for row in entity_str:
                    translated_text = translate.translate_text(
                        Text=row['Text'], SourceLanguageCode="en", TargetLanguageCode=sourceLang)
                    print('en: '+row['Text']+', '+sourceLang +
                          ': '+translated_text['TranslatedText'])
                    text_to_write = translated_text['TranslatedText'].encode(
                        'utf8', 'replace')
                    row['Text'] = text_to_write
                    line = s3_object+',' + \
                        row['Text']+','+row['Category']+',' + \
                        row['Type']+','+str(row['Score'])+'\n'
                    content = content + line

        s3.put_object(Bucket=outputBucket, Key=outputObj, Body=content)
        preURL = s3.generate_presigned_url(
            'get_object', Params={'Bucket': outputBucket, 'Key': outputObj}, ExpiresIn=300)
